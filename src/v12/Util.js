...
module.exports = {
  ...
  resolveMenu(data) {
    ...
    if (!data.custom_id)
      throw new Error('NO_MENU_ID: Please provide menu id');

    let options = this.resolveMenuOptions(data.options);

    if (!options)
      throw new Error('NO_BUTTON_OPTIONS: Please provide at least one menu option');
  ...
  resolveMenuOptions(data) {
    if (!Array.isArray(data))
      throw new Error('INVALID_OPTIONS: The select menu options must be an array');
